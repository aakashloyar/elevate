...
func (r *Repository) Save(ctx context.Context, evaluation domain.Evaluation) error {
	...
	_, err = r.db.ExecContext(ctx, `INSERT INTO evaluations (submission_id, assessment_id, user_id, score, questions, evaluated_at) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (submission_id) DO NOTHING`, evaluation.SubmissionID, evaluation.AssessmentID, evaluation.UserID, evaluation.Score, questions, evaluation.EvaluatedAt)
	return err
}
func (r *Repository) FindBySubmissionID(ctx context.Context, submissionID string) (domain.Evaluation, error) {
	...
	var questions []byte
	err := r.db.QueryRowContext(ctx, `SELECT submission_id, assessment_id, user_id, score, questions, evaluated_at FROM evaluations WHERE submission_id = $1`, submissionID).Scan(&result.SubmissionID, &result.AssessmentID, &result.UserID, &result.Score, &questions, &result.EvaluatedAt)
